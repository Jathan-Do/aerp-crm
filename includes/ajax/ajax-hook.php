...
// AJAX: Thêm nhanh khách hàng (popup modal)
add_action('wp_ajax_aerp_crm_quick_add_customer', 'aerp_crm_quick_add_customer_callback');
function aerp_crm_quick_add_customer_callback()
{
    if (!is_user_logged_in()) {
        wp_send_json_error('Bạn cần đăng nhập.');
    }

    $nonce = sanitize_text_field($_POST['aerp_quick_add_customer_nonce'] ?? $_POST['_wpnonce'] ?? '');
    if (!wp_verify_nonce($nonce, 'aerp_quick_add_customer')) {
        wp_send_json_error('Nonce không hợp lệ.');
    }
    ...
    $result = AERP_Frontend_Customer_Manager::quick_add_customer($data);
    ...
    if (!empty($result['success'])) {
        ...
        wp_send_json_success([
            ...
            'redirect_url' => home_url('/aerp-crm-customers/' . $customer_id),
        ]);
    }

    wp_send_json_error($result['message'] ?? 'Có lỗi xảy ra.');
}
