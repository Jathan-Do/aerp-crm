...
class AERP_Frontend_Customer_Manager
{
    ...
    /**
     * Thêm nhanh khách hàng (chỉ các trường: full_name, address, phone_number, assigned_to, note).
     * Dùng cho form popup AJAX. Các trường khác để mặc định.
     *
     * @param array $data ['full_name', 'address', 'phone_number', 'assigned_to', 'note']
     * @return array ['success' => true, 'customer_id' => int] hoặc ['success' => false, 'message' => string]
     */
    public static function quick_add_customer($data)
    {
        global $wpdb;
        $table = $wpdb->prefix . 'aerp_crm_customers';
        ...
        $phone_number = isset($data['phone_number']) ? preg_replace('/\D+/', '', (string) $data['phone_number']) : '';
        ...
        if (! $validate_vn_phone($phone_number)) {
            return ['success' => false, 'message' => 'Số điện thoại không hợp lệ theo chuẩn Việt Nam.'];
        }
        // Chuẩn hóa lưu dạng 0xxxxxxxxx
        if (str_starts_with($phone_number, '84') && strlen($phone_number) === 11) {
            $phone_number = '0' . substr($phone_number, 2);
        }

        $duplicates = self::check_duplicate_phone_numbers([$phone_number], 0);
        if (! empty($duplicates)) {
        ...
        $format = ['%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%d', '%s', '%d', '%s'];

        $wpdb->insert($table, $data_insert, $format);
        ...
        if (function_exists('aerp_clear_table_cache')) {
            aerp_clear_table_cache();
        }
